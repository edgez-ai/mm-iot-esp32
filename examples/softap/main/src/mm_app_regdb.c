...
{
    printf("Regulatory DB has %u domains\\n", regulatory_db.num_domains);
    for (unsigned i = 0; i < regulatory_db.num_domains; i++)
    {
        printf("  [%u] %s: %u channels\\n", i, 
               regulatory_db.domains[i]->country_code,
               regulatory_db.domains[i]->num_channels);
    }
